...
class OperationGetCustomPhonemizer(OperationNode):
    def __init__(self, request: custom_phonemizer_pb2.PhonemizerId, expected_to_fail: bool = False):
        super().__init__(expected_to_fail)
        self.stub = custom_phonemizer_pb2_grpc.CustomPhonemizersStub(channel=self.channel)
        self.request: custom_phonemizer_pb2.PhonemizerId = request

    def execute_grpc(self) -> Optional[custom_phonemizer_pb2.CustomPhonemizerProto]:
    ...
    def _basic_positive_validate(self) -> None:
        ...
        assert isinstance(self.result, custom_phonemizer_pb2.CustomPhonemizerProto)

    def _basic_negative_validate(self) -> None:
        super(OperationGetCustomPhonemizer, self)._basic_negative_validate()
        assert not isinstance(self.result, custom_phonemizer_pb2.CustomPhonemizerProto)


class OperationCreateCustomPhonemizer(OperationNode):
    def __init__(
            self,
            request: custom_phonemizer_pb2.CreateCustomPhonemizerRequest,
            expected_to_fail: bool = False
    ):
        super().__init__(expected_to_fail)
        self.stub = custom_phonemizer_pb2_grpc.CustomPhonemizersStub(channel=self.channel)
        self.request: custom_phonemizer_pb2.CreateCustomPhonemizerRequest = request

    def execute_grpc(self) -> Optional[custom_phonemizer_pb2.PhonemizerId]:
    ...
    def _basic_positive_validate(self) -> None:
        ...
        assert isinstance(self.result, custom_phonemizer_pb2.PhonemizerId)

    def _basic_negative_validate(self) -> None:
        super(OperationCreateCustomPhonemizer, self)._basic_negative_validate()
        assert not isinstance(self.result, custom_phonemizer_pb2.PhonemizerId)


class OperationDeleteCustomPhonemizer(OperationNode):
    def __init__(
            self,
            request: custom_phonemizer_pb2.PhonemizerId,
            expected_to_fail: bool = False
    ):
        super().__init__(expected_to_fail)
        self.stub = custom_phonemizer_pb2_grpc.CustomPhonemizersStub(channel=self.channel)
...
class OperationUpdateCustomPhonemizer(OperationNode):
    def __init__(self, request: custom_phonemizer_pb2.UpdateCustomPhonemizerRequest,
                 expected_to_fail: bool = False):
        super().__init__(expected_to_fail)
        self.stub = custom_phonemizer_pb2_grpc.CustomPhonemizersStub(channel=self.channel)
        self.request: custom_phonemizer_pb2.UpdateCustomPhonemizerRequest = request

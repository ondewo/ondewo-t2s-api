...
def gen_samples(config_path: str, sentences: List[str], output_dir: Path) -> None:
...
def main() -> None:
    ...
    for config_name in os.listdir(args.config_dir):
        ...
        proc = multiprocessing.Process(target=gen_samples, args=(config_path, sentences, out_dir))
        proc.start()
        proc.join()
